...
GlobalKey<ScaffoldState>myScaffold=GlobalKey<ScaffoldState>();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:  myWaitingCarts==null?myShimmer(context):  Stack(
        children: [
          GridView.builder(
            ...
            itemBuilder:(context,index)=>WaitingCartItem(height: height,cart: myWaitingCarts![index]),
...
class WaitingCartItem extends StatefulWidget {
  final Cart cart;
  final double height;
  const WaitingCartItem({Key? key, required this.cart, required this.height}) : super(key: key);
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Center(
                  child: Container(
                    height: widget.height*.12,width:widget. height*.12,
                    decoration: BoxDecoration(
                      color: Colors.white,
                      image: DecorationImage(image: CachedNetworkImageProvider(widget.cart.product![productImageUrl]),fit: BoxFit.fill),
                ...
                Center(child: Padding(
                  ...
                  child: Text(widget.cart.product![productName],style: TextStyle(fontSize:widget. height*.018,fontWeight: FontWeight.bold),maxLines: 1,overflow: TextOverflow.visible),
                )),


            done?Center(child: Padding(
              padding:  EdgeInsets.symmetric(vertical: widget.height*.03),
              child: Icon(Icons.done_all_outlined,color: Colors.green,size: widget.height*.04,),
            )):
            ...
            Column(
                children: [
                  Padding(
                    padding:   EdgeInsets.symmetric(vertical: widget. height*.01),
                    child: Center(child: Text(widget.cart.colorCode.toString(),style: TextStyle(color: Colors.deepOrange,fontWeight: FontWeight.w900,fontSize: widget.height*.022),)),
                  ),
                  ...
                  Center(
                    child: Container(
                     ...
                     // color: Colors.red,
                      padding: EdgeInsets.symmetric(vertical: widget.height*.005),
                      alignment: Alignment.center,
                      child: Form(
                        key: _myKey1,
                        child: CodePriceTextField(
                          input: TextInputType. number,
                          sFuntion: (s){setState(() {codePrice=double.parse(s!) ;});return null;},
                          vFuntion: (v){if(v!.isEmpty){return'enter price';}return null;},
                          label: 'Color Price',
                  ...
                  Padding(
                    ...
                    child: Center(
                      child: EbottonI(
                        onpressed: ()async{
                          if(!_myKey1.currentState!.validate()){return;}
                          else{
                            ...
                            await Cart().editCart(
                              ...
                              map: {codePricee:codePrice,cartStatus:selling,cartTotalPrice:codePrice!.toDouble()*widget.cart.quantity!}
                        ...
                        child: Text('Done',style: TextStyle(fontSize:widget. height*.016,color: Colors.white)),
                        icon: Icon(Icons.done_all_outlined,size:widget. height*.018,color: Colors.white),
                        elevation: 5,
                        borderRadius: 10,
...
class CodePriceTextField extends StatelessWidget {
