...
class SelectWorkerScreen extends StatefulWidget {
  ...
  const SelectWorkerScreen({Key? key, required this.requestWorker}) : super(key: key);
...
class _SelectWorkerScreenState extends State<SelectWorkerScreen> {
  ...
  getWorkers()async{
   var workers= await Worker().getWorkers(category: widget.requestWorker.category!,cityId: widget.requestWorker.cityId!);
   setState(() {
     workersList=workers;
   });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${getLang(context, "selecting")} ${getLang(context, widget.requestWorker.category!)}'),
      ),


      body:  workersList==null?myShimmer(context):
        Stack(
          children: [
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return workerItem(worker:workersList![index] ,height: height,index:index) ;},
            ...
            Positioned(
              ...
              left: width*.4,
              child: EbottonI(
                child: Text('${getLang(context, "done")}',style: TextStyle(color: Colors.white),),
                ...
                onpressed: ()async{
                  if(selectedIndex==null){
                    MyFlush().showFlush(context: context, text: ' you must select worker');
                    return;
                  }
                  else{

                    RequestWorker().editRequest(id: widget.requestWorker.id!, map: {
                      requestWorkerStatus:requested,
                      requestWorkerWorker:Worker().toMap(workersList![selectedIndex!])
...
int? selectedIndex;
  workerItem({required double height,required Worker worker,required int index}){
    var city=City().getCity(key:cityId, value: worker.cityId!);

    var nameStyle= TextStyle(fontWeight: FontWeight.normal,fontSize: height*.014,color: Colors.black);
    var valueStyle= TextStyle(fontWeight: FontWeight.normal,fontSize: height*.014,color: Colors.black);
      ...
      Padding(
        ...
        child: InkWell(
          ...
          child: Material(
            ...
            color:selectedIndex==index?Colors.green: Theme.of(context).primaryColor,
            elevation: 5,
            child: SizedBox(
              height: height*.22,
                    ...
                    child: Row(
                      ...
                      children:  [
                        Padding(
                          ...
                          child: Text(worker.name!,style: TextStyle(color: Colors.white,fontSize: height*.02),),
                        ),
                        Material(
                          elevation: 5,
                          color: Colors.white,
                            ...
                            children: [
                              Row(
                                children:  [
                                  ...
                                  Text('${getLang(context, worker.category!)}',style: valueStyle,),
                                ],
                              ),
